function [Bg,cg,info,meanw]=weighted_pca(Data,W,components,max_iter,display,Bg,cg,meanw)
...
for i=1:size(Data,2)
      ...
      %cg(:,i)=(Bg'*Ap)\(Ap'*(Data(:,i)-meanw));
      cg(:,i)=pinv(Bg'*Ap)*Ap'*(Data(:,i)-meanw);
end;
...
for i=1:size(Bg,1)
   ...
   %Bg(i,:)=((cg*Ap')\(Ap*(Data(i,:)-meanw(i)*ones(1,size(Data,2)))'))';
   Bg(i,:)=(pinv(cg*Ap')*Ap*(Data(i,:)-meanw(i)*ones(1,size(Data,2)))')';
end
